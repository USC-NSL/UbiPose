...
FLAGS = flags.FLAGS
flags.DEFINE_string('colmap_image_txt', None, "The path to the images.txt of the colmap reconstruction. The reconstruction need to be transformed to the mesh coodinate" )
...
def parseStatsFile(stats):
    stats_df = pd.read_csv(stats, sep=' ', header=None, index_col=False, names=["filename",
                                                                                "vio_est_qvec_0",
                                                                                "vio_est_qvec_1",
                                                                                "vio_est_qvec_2",
                                                                                "vio_est_qvec_3",
...
def main(argv):
    ...
    result_df.rename(columns={0: 'filename', 1: 'error_R',
                     2: 'error_t', 3: 'gt_qvec', 4: 'gt_tvec'}, inplace=True)
    ...
    for index, row in merged_df.iterrows():
        ...
        vio_qvec = [row['vio_est_qvec_0'], row['vio_est_qvec_1'],
                    row['vio_est_qvec_2'], row['vio_est_qvec_3']]
        vio_tvec = [row['vio_est_tvec_0'],
                    row['vio_est_tvec_1'], row['vio_est_tvec_2']]
        ...
        localized_qvec = [row['localized_qvec_0'], row['localized_qvec_1'],
                          row['localized_qvec_2'], row['localized_qvec_3']]
        localized_tvec = [row['localized_tvec_0'],
                          row['localized_tvec_1'], row['localized_tvec_2']]
        ...
        cache_localized_qvec = [row['cache_localized_qvec_0'], row['cache_localized_qvec_1'],
                                row['cache_localized_qvec_2'], row['cache_localized_qvec_3']]
        cache_localized_tvec = [row['cache_localized_tvec_0'],
                                row['cache_localized_tvec_1'], row['cache_localized_tvec_2']]
        ...
        merged_df.at[index,
                     'est_cache_localized_error_q'] = est_cache_localized_error_q
        merged_df.at[index,
                     'est_cache_localized_error_t'] = est_cache_localized_error_t
    
    merged_df['index'] = merged_df.index
    
    merged_df.plot(kind='scatter', x='index', y='est_localized_error_t',
                   ax=ax[2][0], label='est localize error', color='y')
    merged_df.plot(kind='scatter', x='index', y='est_cache_localized_error_t',
                   ax=ax[2][0], label='est cache localize error', color='c')
    merged_df.plot(kind='scatter', x='index', y='localized_error_t',
                   ax=ax[2][0], label='localize error', color='r')
    merged_df.plot(kind='scatter', x='index', y='cache_localized_error_t',
                   ax=ax[2][0], label='cache localize error', color='g')
    merged_df.plot(y='vio_error_t', ax=ax[2][0], color='k')
    merged_df.plot(y='error_t', ax=ax[2][0])
    ax[2][0].set_ylim(ax[1][0].get_ylim())
    
    merged_df.plot(kind='scatter', x='index', y='est_localized_error_q',
                   ax=ax[2][1], label='est localize error', color='y')
    merged_df.plot(kind='scatter', x='index', y='est_cache_localized_error_q',
                   ax=ax[2][1], label='est cache localize error', color='c')
    merged_df.plot(kind='scatter', x='index', y='localized_error_q',
                   ax=ax[2][1], label='localize error', color='r')
    merged_df.plot(kind='scatter', x='index', y='cache_localized_error_q',
                   ax=ax[2][1], label='cache localize error', color='g')
    ...
    print("Number of early exited frames %d" %
          len(stats_df[stats_df.early_exited == 1]))
    print("Percentage of early exited frames %f" %
          (len(stats_df[stats_df.early_exited == 1]) / len(stats_df)))
    print("Number of cache localized frames %d" %
          len(stats_df[stats_df.accepted_cache == 1]))
    print("Percentage of cache localized frames %f" %
